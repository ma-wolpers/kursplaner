...
_HEADERS = ["Datum", "Stunden", "Inhalt", "Thema/Ausfall"]
...
def test_rows_outside_range_untouched():
    ...
    assert saved_rows[0][2] == "[[before]]"
    assert saved_rows[1][2] == "[[new]]"
    assert saved_rows[2][2] == "[[after]]"


def test_ferien_slot_written_with_zero_stunden():
    """Ferien-Slot erhält Stunden=0 und Ausfallnotiz in Thema/Ausfall-Spalte."""
    rows = [["06-01-26", "2", "", ""]]
    uc, repo = _make_uc()
    draft = [_slot(date(2026, 1, 6), stunden=0, is_ferien=True, ausfall_reason="X Ferien")]
    uc.execute(_table(rows), date_from=date(2026, 1, 6), date_to=date(2026, 1, 6), draft_slots=draft)

    row = repo.saved.rows[0]
    assert row[1] == "0"
    assert "Ferien" in row[3]


def test_user_ausfall_slot_written_with_marker():
    """User-Ausfall-Slot erhält Ausfall-Marker in Thema/Ausfall-Spalte."""
    rows = [["06-01-26", "2", "[[abc123]]", ""]]
    uc, repo = _make_uc()
...
def test_splice_adds_new_rows_for_new_dates():
    ...
    assert repo.saved.rows[1][2] == "[[def]]"
