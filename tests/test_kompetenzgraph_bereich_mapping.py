...
def test_parses_inhaltsbereich_with_kuerzel_and_title():
    ...
    node, issues = parse_bereich_node_from_raw(body, node_id="I-Raum und Form", source=make_source_ref("I-Raum und Form"))
...
def test_parses_prozessbereich_from_p_prefix():
    ...
    node, _issues = parse_bereich_node_from_raw(body, node_id="P-Argumentieren", source=make_source_ref("P-Argumentieren"))
...
def test_missing_kuerzel_line_is_soft_warning_with_empty_kuerzel():
    node, issues = parse_bereich_node_from_raw("# Titel ohne Kuerzel", node_id="I-Test", source=make_source_ref("I-Test"))
