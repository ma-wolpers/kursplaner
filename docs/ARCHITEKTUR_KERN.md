...
| `kursplaner/adapters/gui/grid_renderer.py` | ~650 (2026-08-06, vorher ~709: `_is_linked_day` entfernt, delegiert jetzt an `RowDisplayModeUseCase.is_linked_day()`) | Bündelt die weiterhin eng gekoppelte Grid-Render-Logik: Vollaufbau (`_rebuild_grid`, geteilter mutabler Widget-/Layout-Zustand), Patch-Updates (`update_cell`/`update_header`/`update_row_style`/`update_column`/`refresh_grid_content`), Zell-Styling, Marker, Scroll-/Zoom-Handling. Konsequenz aus Architektur-Abschnitt 25f: Grid-Renderer ist eine dedizierte, in sich geschlossene Render-Komponente; eine weitere Aufteilung würde nur denselben mutablen Zustand über Modulgrenzen verteilen. |
| `kursplaner/adapters/gui/overview_controller.py` | ~440 (2026-08-15 nachgetragen; vorher ~426) | Verwaltet Kursübersicht, Detailview-Wechsel, Tageslog und Reload-Flows. Zusammenhängende Zustandsmaschine; Aufspaltung würde zyklische Abhängigkeiten erzeugen. Wuchs 2026-08-09 um den Archivierungs-Trigger in `refresh_overview()`, 2026-08-15 (Architektur-Review, J6) um die Extraktion in eine eigene, benannte `sync_course_storage_lifecycle()`-Methode (reine Verantwortungstrennung, kein Verhaltensunterschied). |
...
| `kursplaner/core/usecases/save_cell_value_usecase.py` | ~346 (2026-08-06 nachgetragen; lag schon vorher bei ~327, unlimitiert dokumentiert) | Ein einziger Save-Flow (`build_edit_plan`/`build_runtime_context`/`build_confirmation_plan`/`execute`) für alle Zellfeld-Typen (inhalt, stunden, YAML-Felder, UB-Entwicklungsfelder, Oberthema-ohne-Link). Die Verzweigungen pro Feldtyp sind der eigentliche Grund für die Länge; eine Aufteilung nach Feldtyp würde den gemeinsamen Save-/Rename-/Confirmation-Ablauf über mehrere Klassen verteilen. Refaktorierungskandidat, falls weitere Feldtypen hinzukommen. |
