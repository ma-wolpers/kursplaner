...
| `kursplaner/adapters/gui/grid_renderer.py` | ~688 effektiv / ~911 roh (2026-08-29 nachgetragen — deutlich mehr Zuwachs seit 2026-08-06 als bisher dokumentiert, hier nur der eigene Beitrag protokolliert, keine vollständige Nachaudit der Zwischenschritte) | Bündelt die weiterhin eng gekoppelte Grid-Render-Logik: Vollaufbau (`_rebuild_grid`, geteilter mutabler Widget-/Layout-Zustand), Patch-Updates (`update_cell`/`update_header`/`update_row_style`/`update_column`/`refresh_grid_content`), Zell-Styling, Marker, Scroll-/Zoom-Handling. Konsequenz aus Architektur-Abschnitt 25f: Grid-Renderer ist eine dedizierte, in sich geschlossene Render-Komponente; eine weitere Aufteilung würde nur denselben mutablen Zustand über Modulgrenzen verteilen. 2026-08-29 um `_grow_row_minsize_for_cell()` gewachsen (Performance-Fix, s. DEVELOPMENT_LOG — `update_column()` patcht jetzt nur noch eine Spalte statt alle Tage über `update_row_style()` zu sweepen). |
| `kursplaner/adapters/gui/overview_controller.py` | ~446 (2026-08-29 nachgetragen; vorher ~440) | Verwaltet Kursübersicht, Detailview-Wechsel, Tageslog und Reload-Flows. Zusammenhängende Zustandsmaschine; Aufspaltung würde zyklische Abhängigkeiten erzeugen. Wuchs 2026-08-09 um den Archivierungs-Trigger in `refresh_overview()`, 2026-08-15 (Architektur-Review, J6) um die Extraktion in eine eigene, benannte `sync_course_storage_lifecycle()`-Methode (reine Verantwortungstrennung, kein Verhaltensunterschied), 2026-08-29 um den optionalen `changed_row_indices`-Parameter auf `collect_day_columns()` (Performance-Fix, s. DEVELOPMENT_LOG — leitet an `build_day_columns_incremental()` weiter statt an einen vollständigen Rebuild). |
| `kursplaner/adapters/gui/selection_controller.py` | ~350 effektiv / ~495 roh (2026-08-29 nachgetragen; vorher ~314, weiterer Zuwachs dazwischen nicht nachauditiert) | Durch Fast-Path-Erweiterung für Grid-Navigation 2026-08-04 minimal über Limit. Refaktorierungskandidat: `refresh_header_styles`/`_apply_single_header_style` gehört semantisch in `grid_renderer.py`. 2026-08-29 um einen weiteren Fast Path gewachsen (`_restyle_headers_for_selection_change()`, Performance-Fix s. DEVELOPMENT_LOG — Spaltenselektion stylt jetzt nur die betroffenen Header statt aller). |
| `kursplaner/adapters/bootstrap/wiring.py` | ~451 (2026-08-04 nachgetragen; bestand schon vorher) | Composition Root (siehe `docs/ARCHITEKTUR_EINFACH.md` Abschnitt 3): verkabelt bewusst an genau einer Stelle alle konkreten Repositories mit allen Use Cases. Die Zeilenzahl wächst linear mit der Anzahl der Use Cases, nicht mit Komplexität pro Zeile; eine Aufteilung würde die zentrale Verdrahtung gerade auflösen, die dieses Modul laut Architektur bewusst leisten soll. |
| `kursplaner/adapters/gui/screen_builder.py` | ~969 (2026-08-06 nachgetragen; bestand schon vorher) | Baut Layout, Menüs und Toolbar des Hauptfensters auf und bündelt zugleich die zentrale Shortcut-Runtime (Keybinding-Registry, Mode-Kontext-Ableitung, Popup-Policy-Tracking). Diese Bausteine sind eng gekoppelt, da jede Shortcut-Bindung denselben Mode-Kontext benötigt; eine Aufteilung würde Bindungsdeklaration und die Laufzeitauswertung, von der ihre Korrektheit abhängt, über Modulgrenzen trennen. Refaktorierungskandidat: Shortcut-Runtime (Keybinding/Popup-Registry, `_build_runtime_context`, `_bind_runtime_shortcut`) in ein eigenes Modul auslagern. |
| `kursplaner/adapters/gui/ui_intent_controller.py` | ~836 (2026-08-08 nachgetragen; vorher ~834) | Zentraler Dispatcher für alle UI-Intents des Hauptfensters (Grid-Navigation, Selektion, Escape-/Commit-Eskalation, Spalten-/Zeilenoperationen) — eine einzige Übersetzungsschicht von UI-Ereignissen auf `ui_state`-Übergänge. Aufspaltung nach Intent-Gruppe würde die gemeinsame State-Machine (`selection_level`-Übergänge) über mehrere Klassen verteilen, die alle denselben `ui_state` mutieren. |
| `kursplaner/adapters/gui/timetable_change_dialog.py` | ~323 (2026-08-09 nachgetragen; vorher ~352, durch Rhythmus-Umstellung auf `WeekdayRhythmPicker`/`_infer_day_hours`-Entfernung leicht geschrumpft) | Split-View-Dialog mit eng gekoppeltem Entwurfs-Zustand (`_draft_slots`, Undo-Stack) und Tk-Widgets, die synchron zu diesem Zustand bleiben müssen. Kein Daten/Logik-Mix (keine auslagerbare Datentabelle wie bei `path_store.py`); eine echte Aufspaltung würde eine Presenter-/State-Abstraktion getrennt vom Tk-Widget-Code erfordern — ein größerer, hier nicht angegangener Refactor. Refaktorierungskandidat: Slot-Bearbeitungsoperationen (`_on_swap`/`_place_displaced_content`/`_on_remove`/Undo-Stack) in eine von Tk unabhängige `DraftSlotEditor`-Klasse auslagern, vom Dialog nur noch über deren Ergebnis-Zustand rendern lassen. |
| `kursplaner/adapters/gui/lesson_conversion_controller.py` | ~523 (2026-08-09 nachgetragen; lag schon vor dieser Session über dem Limit, bisher ohne Ausnahme-Eintrag) | Bündelt alle Umwandlungs-Flows einer Planzeile (Unterricht/Ausfall/Hospitation/LZK) inkl. Prefill-Ableitung, UB-Impuls-Ladung und Fachquellen-Auflösung — ein zusammenhängender Adapter-Ablauf pro Zielzustand. Refaktorierungskandidat (im Umsetzungsplan vermerkt): `_unterricht_prefill_values`/`_coerce_string_list`/`_extract_markdown_section_refs`/`_prefill_ausfall_reason_from_content` in ein eigenes `lesson_conversion_prefill.py` auslagern. |
