...
| `kursplaner/adapters/gui/ui_intent_controller.py` | ~834 (2026-08-06 nachgetragen; bestand schon vorher) | Zentraler Dispatcher für alle UI-Intents des Hauptfensters (Grid-Navigation, Selektion, Escape-/Commit-Eskalation, Spalten-/Zeilenoperationen) — eine einzige Übersetzungsschicht von UI-Ereignissen auf `ui_state`-Übergänge. Aufspaltung nach Intent-Gruppe würde die gemeinsame State-Machine (`selection_level`-Übergänge) über mehrere Klassen verteilen, die alle denselben `ui_state` mutieren. |
| `kursplaner/adapters/gui/timetable_change_dialog.py` | ~352 (2026-08-08 nachgetragen; lag schon vorher bei ~343) | Split-View-Dialog mit eng gekoppeltem Entwurfs-Zustand (`_draft_slots`, Undo-Stack) und Tk-Widgets, die synchron zu diesem Zustand bleiben müssen. Kein Daten/Logik-Mix (keine auslagerbare Datentabelle wie bei `path_store.py`); eine echte Aufspaltung würde eine Presenter-/State-Abstraktion getrennt vom Tk-Widget-Code erfordern — ein größerer, hier nicht angegangener Refactor. Refaktorierungskandidat: Slot-Bearbeitungsoperationen (`_on_swap`/`_place_displaced_content`/`_on_remove`/Undo-Stack) in eine von Tk unabhängige `DraftSlotEditor`-Klasse auslagern, vom Dialog nur noch über deren Ergebnis-Zustand rendern lassen. |
