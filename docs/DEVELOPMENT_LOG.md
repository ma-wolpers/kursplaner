...
- Soft-Guardrail fuer Settings-Shortcut-Abdeckung ergaenzt: `tools/ci/check_ai_guardrails.py` meldet lokal (non-blocking) eine Warnung, wenn `OPEN_SETTINGS` gefunden wird, aber kein Strg+,-Binding (`<Control-comma>`/`<Control-,>`) in Runtime-/Shortcut-Quellen vorhanden ist.
