...
**Ableitung statt Doppelpflege**: `QueryUbAchievementsUseCase._build_course_stufe_map()` baut, analog zu `QueryUbPlanUseCase._build_course_map()`, ein `lesson_stem -> Stufe`-Mapping ueber alle Kurstabellen unter dem Unterrichtsordner (`plan_repo.load_plan_tables()`, "Inhalt"-Spalte). Eine UB referenziert ihre Einheit bereits ueber das bestehende `Einheit`-Feld (Wiki-Link); `execute()` loest darueber live die Stufe des zugehoerigen Kurses auf (`parse_jahrgangsstufe(table.metadata.get("Stufe"))`) statt ein UB-eigenes Feld zu lesen. Kein Registry-/Cache-Mechanismus (waere ein Frische-Risiko) — die Ableitung laeuft bei jedem Oeffnen der Achievements-Ansicht neu, wie bereits bei `QueryUbPlanUseCase` etabliert.

**Eindeutigkeits-Guardrail**: Eine Einheit gehoert strukturell genau einem Kurs. `_build_course_stufe_map()` prueft das aktiv (anders als `_build_course_map()`s stillschweigendes `setdefault`/"erster gewinnt"): ist derselbe `lesson_stem` in zwei verschiedenen Kursen verlinkt, wirft die Methode `RuntimeError` mit Klartext-Diagnose, statt eine falsche Stufe zu liefern. Faengt sich im bestehenden GUI-Fehlerpfad (`show_ub_achievements_view`s generisches `except Exception` → `messagebox.showerror`).

**Feld komplett entfernt** (nicht nur Default-Vorbefuellung): `UB_YAML_KEY_JAHRGANGSSTUFE`-Konstante, das Dialogfeld in `ub_mark_dialog.py`, der `jahrgangsstufe`-Parameter durch `MarkUnitAsUbUseCase.execute()`/`UbMarkDialogResult` — alles entfernt. `parse_jahrgangsstufe`/`JAHRGANGSSTUFE_MIN`/`MAX` bleiben (wenden sich jetzt auf den Kurs-`Stufe`-Wert an, Docstring entsprechend umformuliert). Altbestand mit noch vorhandener `Jahrgangsstufe:`-Zeile laedt weiterhin fehlerfrei (`_UB_SCHEMA` prueft nur `required_keys`/`non_empty_keys`, kein geschlossenes Schluesselset) und verliert die Zeile beim naechsten Speichern automatisch, da sie nicht mehr in `_UB_ORDERED_KEYS` steht — keine Backfill-Migration noetig.

**Neue gemeinsame Praesentationsabstraktion**: `group_achievements_by_domain()` (+ `AchievementDomainGroup`) in `query_ub_achievements_usecase.py` — reine, Framework-unabhaengige Funktion, die "nach Fach gruppiert (Reihenfolge `DOMAIN_ORDER`), je Gruppe nach Erfuellungsgrad absteigend sortiert" genau einmal definiert. Vorbereitet fuer zwei Konsumenten (GUI-Fachgruppierung, PDF-Report), die beide in einem separaten Schritt darauf umgestellt werden, statt Gruppierung/Sortierung unabhaengig voneinander zu implementieren. Aendert nichts an der bestehenden Sortierung von `QueryUbAchievementsUseCase.execute()`.

**Tests**: `test_query_ub_achievements_usecase.py` komplett auf das neue Kurs-Mapping umgestellt (`_FakePlanRepo`/`_plan_repo_for()` bauen je Stufe einen synthetischen Kurs); neue Faelle: Ableitung aus dem Kurs statt UB-Feld, ein noch vorhandenes Alt-Feld auf der UB wird ignoriert (Kurs gewinnt immer), Duplicate-`lesson_stem`-Mapping wirft `RuntimeError`. `test_ub_repository.py`: neuer Migrationstest (Alt-Datei mit `Jahrgangsstufe:` laedt, verliert Feld beim Resave). `test_mark_unit_as_ub_usecase.py`: Assertion auf geschriebenes Jahrgangsstufe-Feld ersetzt durch Abwesenheits-Check. Neue Datei `test_group_achievements_by_domain.py` fuer die Gruppierungsfunktion. 684/684 Tests gruen.

### Added (2026-08-30) — Sequenzfeld-Spannzellen virtualisiert; Gap-Verdacht geprüft und widerlegt (Kursplaner Item 4, Stufe 6 — Abschluss)

**Anlass**: letzte Stufe der Viewport-Virtualisierung, laut Plan die "trickiest correctness surface" -- Sequenzziel-/Leitkompetenz-Zellen sind keine "eine Zelle pro Tag", sondern überspannen via `columnspan` mehrere Tages-Spalten gleichzeitig.

**Design-Korrektur gegenüber dem ursprünglichen Plan-Entwurf**: der dort noch vorgesehene Force-Commit über `save_sequence_field()` beim Evicten entfällt ersatzlos -- Stufe 4 hat dasselbe Prinzip für normale Zellen bereits zugunsten von `pending_cell_text` verworfen (Force-Commit beim Scrollen verletzt die harte Anforderung "Scrollen speichert nie"); dieselbe Entscheidung gilt konsequent auch für Sequenzfelder. `pending_cell_text` wird direkt mitgenutzt (Schlüssel `(field_key, first_row_index)`, eigener Namensraum ohne Kollisionsrisiko, da "Sequenzziel"/"Leitkompetenz" nie als Feld-Schlüssel normaler Zeilen vorkommen).

**Vorbereitende Extraktion** (`sequence_field_grid_renderer.py`): `SEQUENCE_FIELD_ROW_ORDER`-Konstante (einzige Quelle der Wahrheit für den bisher zweifach hart kodierten, aber deterministischen `row_idx` von Sequenzziel/Leitkompetenz) und `compute_row_index_to_grid_col()` (aus `render()` herausgezogen, jetzt von `render()` UND der neuen Reconciliation gemeinsam genutzt, garantiert identische Abbildung statt zweier potenziell divergierender Kopien) -- verifiziert verhaltensgleich (745 Tests unverändert grün vor der eigentlichen neuen Logik).

**`GridRenderer._reconcile_sequence_field_mounts()`** (neu, aufgerufen aus `_reconcile_column_mounts()`, damit nie einer der beiden Reconciliation-Pfade vergessen wird, wenn sich der andere ändert): der volle logische Span eines Sequenzlaufs (`_run_span()`, unverändert) wird mit dem in Grid-Spalten übersetzten Mount-Fenster geschnitten (`sub_span = full_span ∩ mounted_col_range`) -- **bei jedem Aufruf neu berechnet, nie fortgeschrieben**. Leerer `sub_span` → Eviction (`_evict_sequence_field_to_cold()`, spiegelt `_evict_cell_to_cold()`: ungespeicherten Text nach `pending_cell_text` retten, `destroy()`); nicht-leerer `sub_span` bei bereits existierendem Widget → nur neu `.grid()`en, falls sich `column`/`columnspan` geändert haben (kein unnötiger Tcl-Call); bei fehlendem Widget → Neu-Erzeugung, aus `pending_cell_text` vorbelegt falls vorhanden, sonst dem Domain-Wert. Sequenzfeld-Zellen tragen kein `active_editor`-Äquivalent (Fokus-/Klick-Intents sind bewusste No-ops), nichts dort zu bereinigen.

**Zwischenzeitlicher Verdacht, geprüft und widerlegt**: während der Umsetzung kam der Verdacht auf, das Verkleinern/Entfernen einer Spannzelle könnte sichtbare Lücken hinterlassen, da die ursprüngliche (statische) Leerzellen-Logik in `_render_one_row()` (für Tage, die zu KEINER Sequenz gehören) nie für dynamisches Mounten gebaut wurde. Auf Nutzerwunsch vor der Umsetzung in einer eigenen Planungsrunde genau hergeleitet: Eine Grid-Spalte wird von der statischen Leerzellen-Erzeugung nur dann ausgeschlossen, wenn sie Teil des `full_span` einer Sequenz ist. Liegt dieselbe Spalte zusätzlich im aktuellen Mount-Fenster, ist sie per Definition der Schnittmenge zwangsläufig auch in `sub_span` enthalten -- es kann keine Spalte geben, die gleichzeitig "Sequenz-Territorium", "im Fenster" und "nicht durch das Sequenz-Widget abgedeckt" ist. Spalten außerhalb des Fensters sind für keine Zeile gemountet (normale Zellen werden dort durch dieselbe Fenstergrenze ebenfalls evictet), daher entsteht dort keine Inkonsistenz zwischen Zeilen. **Kein Leerzellen-Verwaltungsmechanismus gebaut** -- hätte eine Ursache gelöst, die es nicht gibt.

**Tests** (`tests/test_grid_renderer_sequence_field_mount.py`, 16 Fälle): 13 Fälle isolierte Mechanik (Eviction, Dirty-Rettung, Respan-statt-Recreate, No-op-Guards, Remount aus Pending/Domain-Wert, voller Rundlauf) -- reine Logik-Fälle stub-basiert ohne Tk-Root, Remount-Fälle mit echtem off-screen `tk.Tk()`. Zusätzlich, auf Nutzer-Feedback hin bewusst NICHT als Nachrechnung derselben Mengenoperation, sondern am **beobachtbaren Ergebnis** (`sequence_field_widgets` + echtes `grid_info()`): ein durchgehender Szenario-Test über 7 real überlappende Fenster-Übergänge (rechts abgeschnitten, beidseitig abgeschnitten, links abgeschnitten, echte Nicht-Sequenz-Lücke zwischen zwei Läufen, ein Lauf vollständig sichtbar während der andere evictet ist, beide Läufe vollständig außerhalb, Wiedereintritt) beweist am tatsächlichen Mount-Zustand nach jedem Schritt, dass nie eine Lücke oder ein Waisen-Widget entsteht; ein Idempotenz-Test (dasselbe Fenster zweimal hintereinander: keine neuen Widgets, keine erneute `grid()`-Platzierung, Widget-Identität stabil); ein struktureller Kein-Save-Test über dieselbe volle Fenster-Folge (App-Stub ohne `save_sequence_field`/`editor_controller`, würde bei Persistenz-Berührung mit `AttributeError` crashen). Volle Suite: 761/761 (758 vorher + 3 neue Testfunktionen, die zusammen 16 statt vormals 13 Fälle in dieser Datei abdecken).

**Damit ist Kursplaner Item 4 (Viewport-Virtualisierung) vollständig abgeschlossen** -- alle sechs Stufen umgesetzt, getestet, committet.

### Added (2026-08-29) — Echte COLD-Eviction via destroy() + pending_cell_text (Kursplaner Item 4, Stufe 4)

**Anlass**: Höchste Risikostufe des gesamten Virtualisierungs-Plans -- die einzige Stufe, in der tatsächlich Widgets zerstört werden und damit Daten verloren gehen könnten, wenn die Mechanik falsch ist. Baut direkt auf Stufe 3 auf (Mount-Fenster-Logik, `_reconcile_column_mounts()` existiert bereits) und ersetzt deren `grid_remove()`-Zwischenlösung durch echtes `destroy()`.

**Design-Klarstellung gegenüber der ursprünglichen Stufen-Beschreibung**: HOT und WARM werden NICHT als zwei unterscheidbare Zustände geführt (sichtbar vs. `grid_remove()`t), sondern als EIN gemeinsames, immer `grid()`'tes Mount-Fenster. Grund: die Reconciliation läuft asynchron/gedebouncet (`HorizontalViewportSync.on_view_changed()`, 40ms, Stufe 3). Würde der Pufferbereich `grid_remove()`t, könnte eine Spalte, die durch einen kleinen Scroll neu ins Sichtfeld rückt, für die Dauer des Debounce-Fensters als leere Lücke erscheinen -- die Canvas-Sichtbarkeit hängt nur vom Scroll-Offset ab, nicht vom `grid()`/`grid_remove()`-Zustand der Zelle (Spaltenbreiten sind über `grid_columnconfigure()` unabhängig von Zellinhalt reserviert). `grid_remove()` hat ab dieser Stufe deshalb keine Rolle mehr im Mechanismus: alles innerhalb des Mount-Fensters bleibt durchgängig `grid()`'t, alles ausserhalb wird jetzt echt COLD.

**Neue, vom Widget entkoppelte Struktur** `self.pending_cell_text: dict[(field_key, day_index), str]` (`main_window.py`, analog zu den bereits bestehenden koordinatenbasierten `selected_cell`/`active_editor`). Wird ausschliesslich befüllt, wenn eine Zelle mit vom Domain-Wert abweichendem Text `destroy()`t wird (`_evict_cell_to_cold()`) -- ein reines Lesen-und-Merken, **kein I/O, kein `save_cell()`-Aufruf, keine Domain-Mutation**. Geleert wird ausschliesslich bei einem erfolgreichen echten Commit (`editor_controller.save_cell()`, jetzt an beiden `return True`-Stellen inkl. dem "Wert stimmt bereits mit Domain überein"-Frühausstieg, da dort ein veralteter Pending-Eintrag sonst nie verschwinden würde) -- niemals durch Scrollen.

**`_evict_cell_to_cold()`** prüft zusätzlich `ui_state.active_editor`: ein Nutzer kann eine Zelle fokussiert halten und trotzdem den Viewport wegscrollen (Tk-Fokus hängt nicht von Sichtbarkeit ab), und `<FocusOut>` feuert nicht zuverlässig vor `destroy()` -- der State wird deshalb hier explizit bereinigt statt sich auf das Event zu verlassen. `selected_cell` (Navigations-Markierung) wird bewusst NICHT angefasst: ihre Gültigkeit ist eine reine Domain-Frage (bereits seit Stufe 2 unabhängig von Widget-Präsenz), sie bleibt über COLD hinweg korrekt bestehen und wird beim Remount automatisch wieder korrekt gestylt.

**Remount**: `_reconcile_column_mounts()` erweitert um einen zweiten Durchlauf -- für jedes domain-sichtbare (Feld, Tag) innerhalb des Fensters ohne existierendes Widget wird eines neu erzeugt, mit `pending_cell_text` als Vorbelegung falls vorhanden, sonst dem aktuellen Domain-Wert. **Wichtige Vorab-Refaktorierung**, um das ohne Code-Duplikation zu ermöglichen: die komplette Pro-Zelle-Erzeugungslogik aus `_rebuild_grid()`s Schleife (Theming, Editierbarkeit, alle Event-Bindings, `cell_widgets`-Registrierung -- ca. 70 Zeilen) wurde in eine neue, gemeinsame Methode `_create_and_mount_cell()` extrahiert, die JETZT von beiden Pfaden (voller Rebuild UND Einzelzellen-Remount) verwendet wird -- garantiert identisches Zellen-Setup statt zweier potenziell divergierender Kopien. `self.app.day_grid_columns` (Tag → Grid-Spalte) wird dafür jetzt persistiert statt nur lokal in `_rebuild_grid()` zu existieren (analog zu `day_column_x_positions`); `row_idx` wird über das bereits bestehende `_row_index_for_field()` aufgelöst (liest die immer vollständig gemountete Feld-Label-Zeile).

**Speichergrenze jetzt real wirksam**: `cell_widgets` bleibt ab dieser Stufe tatsächlich auf das Mount-Fenster begrenzt (`R × (V + 2·B)` statt `R × Gesamtspaltenzahl`, s. Plan) -- vorher (Stufen 1-3) blieb trotz aller Vorbereitung jede domain-sichtbare Zelle im Speicher.

**Tests**: `tests/test_grid_renderer_column_mount.py` komplett neu geschrieben (11 Fälle) -- reine Eviction-Szenarien (kein Remount nötig) bleiben Stub-basiert ohne echten Tk-Root; Remount-Szenarien (echte Widget-Erzeugung über `_create_and_mount_cell()`) brauchen einen echten, off-screen positionierten Tk-Root (Kartograph-Konvention). Deckt ab: dirty Zelle wird vor Eviction gerettet, clean Zelle bekommt keinen Pending-Eintrag, `active_editor`-Bereinigung bei Treffer/Nicht-Treffer, beide No-op-Guards, bereits gemountete Zelle wird nur regridded statt neu erzeugt, Remount seedet aus `pending_cell_text` bzw. Domain-Wert, und -- strukturell erzwungen -- kein Persistenzpfad wird je berührt (App-Stub ohne `save_cell`/`editor_controller`, würde mit `AttributeError` crashen). Neue `tests/test_editor_controller_pending_text.py` (4 Fälle) für `save_cell()`s neues Clearing-Verhalten: erfolgreicher Commit löscht den Eintrag, der "unverändert"-Frühausstieg löscht einen veralteten Eintrag ebenfalls, ein fehlgeschlagener Save lässt den Eintrag bewusst bestehen, unabhängige Einträge anderer Zellen bleiben unberührt. Volle Suite: 744/744 (735 vorher + 9 neue).

### Added (2026-08-29) — HOT/WARM-Zellen-Mounting via grid()/grid_remove() (Kursplaner Item 4, Stufe 3)

**Anlass**: Erste Stufe der Virtualisierung, die tatsaechlich Laufzeitverhalten aendert (Stufen 0-2 waren rein strukturelle Vorbereitung ohne Verhaltensaenderung). Explizit auf `grid_remove()`/`grid()` beschraenkt, kein `destroy()` -- damit ist diese Stufe von vornherein frei von jedem Datenverlust-Risiko: `grid_remove()` entfernt ein Widget nur aus dem Tk-Geometrie-Manager, das Objekt (Textinhalt, Undo-Stack) bleibt vollstaendig erhalten, `grid()` ohne Argumente stellt exakt dieselbe Platzierung wieder her (natives Tk-Verhalten).

**Wichtige Zwischenerkenntnis beim Entwerfen**: der ursprünglich für diese Stufe vorgesehene `pending_cell_text`-Mechanismus (ungespeicherten Zellentext unabhaengig vom Widget zwischenspeichern) wurde bewusst auf die naechste Stufe verschoben. Sein einziger Zweck ist, Text vor einem `destroy()` zu retten -- bei reinem `grid_remove()` bleibt das Widget-Objekt aber vollstaendig am Leben, es gibt nichts zu retten und keinen Remount-Codepfad, der ihn lesen würde. Ihn hier schon zu bauen hätte ungenutzten, nicht sinnvoll testbaren Code bedeutet; er entsteht jetzt zusammen mit der `destroy()`-Stufe, wo er eine echte Aufgabe hat und gegen ein reales Szenario getestet werden kann, statt gegen ein künstlich simuliertes.

**Neu**: `GridRenderer._reconcile_column_mounts()` -- iteriert `cell_widgets`, ruft `grid()` fuer Zellen innerhalb des aktuellen Mount-Fensters (`viewport_sync_h.mounted_day_index_range()`), `grid_remove()` fuer alle anderen. `grid()`/`grid_remove()` sind idempotent, deshalb bewusst ohne zusaetzliche Zustands-Buchfuehrung -- ein voller Durchlauf pro Aufruf. Nur `cell_widgets` betroffen, Header (immer vollstaendig gemountet) und `sequence_field_widgets` (bewusst isolierte, spaetere Stufe) bleiben unangetastet. Guards: No-op waehrend eines laufenden Rebuilds, No-op wenn kein Mount-Fenster berechenbar ist (Grid noch nicht layoutet).

**Anbindung**: `HorizontalViewportSync.on_view_changed()` (Stufe 1) plant jetzt zusaetzlich zur bereits synchronen Scrollbar-/Header-Synchronisation einen gedebouncten Aufruf von `_reconcile_column_mounts()` (40ms, exakt nach dem Muster von `GridRenderer._schedule_zoom_rebuild()`) -- ein schneller Drag-Scroll erzeugt viele `on_view_changed()`-Ticks pro Sekunde, ohne Debounce wuerde jeder Tick synchron ueber die gesamte `cell_widgets`-Map laufen. `_rebuild_grid()` ruft die Reconciliation zusaetzlich einmal synchron am Ende auf (nach `_on_grid_inner_configure()`, damit `bbox()`/Scrollregion bereits aktuelle Geometrie hat) -- sonst blieben nach einem frischen Rebuild alle Spalten sichtbar, bis der naechste Scroll-Event feuert.

**Harte Anforderung strukturell erzwungen, nicht nur behauptet**: `_reconcile_column_mounts()` beruehrt weder `active_editor` noch Fokus noch `save_cell()` -- der neue Test-Stub in `tests/test_grid_renderer_column_mount.py` enthaelt bewusst KEIN `save_cell`/`editor_controller`-Attribut; ein versehentlicher Save-Aufruf wuerde dort mit `AttributeError` crashen statt still zu bestehen.

**Tests**: `tests/test_grid_renderer_column_mount.py` (6 Faelle, neu, isoliert per `object.__new__(GridRenderer)`, Stub-Stil nach `test_toolbar_geometry_guard.py`) -- Zellen innerhalb/ausserhalb des Fensters, Widget-Identitaet bleibt ueber die Reconciliation stabil (kein Destroy/Recreate), beide No-op-Guards, der strukturell erzwungene "kein Save"-Test. `tests/test_horizontal_viewport_sync.py` um 3 Faelle erweitert (Debounce-Mechanik: Reconciliation wird geplant, nicht synchron ausgefuehrt; schnelle Ticks kollabieren zu einem Timer; der geplante Callback ruft tatsaechlich den `GridRenderer` auf). Volle Suite: 735/735 (726 vorher + 9 neue).

### Changed (2026-08-29) — Dreiwertige `_grid_structure_matches_state()` (Kursplaner Item 4, Stufe 2)

**Anlass**: Zweite, letzte noch verhaltensgleiche Stufe vor dem eigentlichen Mount/Unmount-Risiko (Stufe 3+). `_grid_structure_matches_state()` prüfte bisher binär "domain-sichtbar == hat Widget" für JEDE (Feld, Tag)-Kombination und löste bei jeder Abweichung einen vollen `_rebuild_grid()` aus. Nach der geplanten Virtualisierung wird "domain-sichtbar, aber ausserhalb des Mount-Fensters kein Widget" ein routinemaessiger, korrekter Zustand -- die alte Prüfung wuerde das faelschlich als strukturellen Fehler werten und bei jedem Scroll einen unnoetigen Voll-Rebuild ausloesen, genau das Gegenteil dessen, was Virtualisierung erreichen soll.

**Fix**: die Prüfung bleibt fuer "nicht mehr sichtbar, aber Widget vorhanden" (verwaistes Widget) unveraendert strikt -- das ist immer ein Fehler, unabhaengig vom Mount-Fenster. Fuer "sichtbar, aber kein Widget" gilt das nur noch als Fehler, wenn die Zelle INNERHALB des aktuellen Mount-Fensters liegt (`self.app.viewport_sync_h.mounted_day_index_range()`, aus Stufe 1); ausserhalb des Fensters ist das jetzt erlaubt. Degradiert defensiv auf die alte volle Pruefung, wenn `viewport_sync_h` fehlt oder `mounted_day_index_range()` `None` liefert (Grid noch nicht layoutet) -- damit bleiben leichtgewichtige Test-Stubs ohne echten `HorizontalViewportSync` unveraendert funktionsfaehig. **Ist eine strikte Lockerung, keine Verhaltensaenderung**: alles, was die alte Gleichheitspruefung erfuellte, erfuellt auch die neue -- solange kein echtes Mount/Unmount existiert (vor Stufe 3), bleibt `cell_widgets` fuer jede domain-sichtbare Zelle ohnehin vollstaendig befuellt, das Mount-Fenster wirkt sich also praktisch noch nicht aus.

**Tests**: `tests/test_grid_structure_matches_state.py` (7 Faelle, neu, isoliert per `object.__new__(GridRenderer)`) -- deckt beide Richtungen ab (verwaistes Widget bleibt immer ein Fehler; fehlendes Widget ist nur innerhalb des Fensters ein Fehler, ausserhalb nicht mehr -- der eigentliche Verhaltensunterschied), plus die Fallback-Pfade (kein `viewport_sync_h`, `mounted_day_index_range()` liefert `None`) und die unveraenderten Sanity-Checks (Header-Anzahl, Row-Label-Liste). Bestehende Item-2-Fast-Path-Guard-Tests bleiben unveraendert gruen (Abnahmekriterium laut Plan). Volle Suite: 726/726 (719 vorher + 7 neue).

### Added (2026-08-29) — Zentrale horizontale Viewport-Synchronisation: `HorizontalViewportSync` (Kursplaner Item 4, Stufe 1)

**Anlass**: Erste Stufe der geplanten Viewport-Virtualisierung (Kursplaner Item 4, bedingte Folgearbeit) -- reine, verhaltensgleiche Zentralisierung ohne jedes Mount/Unmount-Risiko, als risikoarmer erster Schritt. Anders als die vertikale Achse (`GridViewportSync`, bereits durch `tests/test_vertical_scroll_architecture_guard.py` erzwungen zentralisiert) war die horizontale Achse auf DREI unabhaengige Stellen verstreut: `grid_renderer.py::_on_horizontal_scroll()`, ein inline-Closure `_on_grid_xscroll` in `screen_builder.py`, und `selection_controller.py::ensure_column_visible()`s direkter `xview_moveto()`-Aufruf. Beim Schreiben des neuen Guard-Tests (s.u.) kamen zwei WEITERE, vom vorherigen Audit nicht erfasste direkte Zugriffe zum Vorschein: ein `grid_canvas.xview_moveto(...)` in `overview_controller.py::_scroll_to_next_unit()` und ein `grid_canvas.xview()`-Lesezugriff in `ensure_column_visible()` selbst -- genau der Fall, fuer den ein automatisierter Guard-Test gebaut wird, statt sich auf eine manuelle Bestandsaufnahme zu verlassen.

**Neue Klasse `HorizontalViewportSync`** (`grid_viewport_sync.py`, gleiche Datei wie `GridViewportSync` -- der bestehende Guard-Test-Allowlist-Mechanismus ist auf Dateiname geschlüsselt, deckt eine zweite Klasse in derselben Datei also automatisch mit ab): `xview()`/`xview_scroll()`/`xview_moveto()`/`xview_range()` nach exaktem Vorbild von `GridViewportSync`s vertikalen Pendants. `on_view_changed(first, last)` ersetzt den bisherigen `_on_grid_xscroll`-Closure als `xscrollcommand` des Grid-Canvas (Tk ruft das bei jeder horizontalen Positionsaenderung auf, unabhaengig von der Quelle) -- dafuer wird der bisher rein lokale `x_scroll`-Scrollbar jetzt als `self.app.x_scroll` gehalten, analog zu den bereits als App-Attribute gehaltenen Canvases.

**`visible_day_index_range()`/`mount_window()`/`mounted_day_index_range()`**: reine, bereits jetzt isoliert testbare Funktionen fuer die eigentliche Virtualisierung (Stufe 3+) -- berechnen die sichtbaren bzw. zu mountenden Tage-Indizes aus `xview_range()` + `day_column_x_positions` + `day_column_width`, dieselbe Pixel-Quelle, die `ensure_column_visible()` bereits nutzt. **Bewusst schon jetzt gebaut, aber noch ungenutzt** (kein Aufrufer verlangt aktuell ein Mount-Fenster) -- der Plan sieht das ausdruecklich so vor, da diese pure Logik unabhaengig vom riskanteren Mount/Unmount-Mechanismus entwickelt und getestet werden kann. `buffer_columns=3` (HOT+WARM-Puffer) ist der im Plan festgelegte Standardwert.

**Neuer Architektur-Guard** `tests/test_horizontal_scroll_architecture_guard.py`, exaktes Gegenstueck zu `test_vertical_scroll_architecture_guard.py` (grept `adapters/gui/*.py` nach direkten `grid_canvas.xview*`/`header_canvas.xview*`-Aufrufen außerhalb von `grid_viewport_sync.py`). Deckte sofort die zwei oben genannten, nicht migrierten Altstellen auf (`overview_controller.py`, `selection_controller.py`) -- beide auf `self.app.viewport_sync_h` umgestellt.

**Tests**: `tests/test_horizontal_viewport_sync.py` (11 Faelle, neu, Stub-Stil wie `test_grid_viewport_sync.py`) -- deckt `xview`/`xview_scroll`/`xview_moveto`(inkl. Clamping)/`xview_range`/`on_view_changed` sowie `visible_day_index_range()`/`mount_window()`/`mounted_day_index_range()` inkl. Rand- und Pufferklemmung ab. Bestehende Tests (`test_selection_controller.py`, `test_row_filter_navigation.py`) um `_HorizontalViewportSyncStub`/`viewport_sync_h` ergaenzt, da `ensure_column_visible()` jetzt darueber statt direkt ueber `grid_canvas` laeuft. Volle Suite: 719/719 gruen (708 vorher + 11 neue).

### Changed (2026-08-29) — Architektur-Audit: `cell_widgets`-Zugriffe von Widget-Praesenz auf Domain-Fragen umgestellt (Kursplaner Item 4 Vorarbeit)

**Anlass**: Vorarbeit fuer die geplante Viewport-Virtualisierung (Item 4, bedingte Folgearbeit). Bevor irgendein Mount/Unmount-Mechanismus gebaut wird, muss `cell_widgets` von "immer vollstaendig" auf "kann legitim eine Teilmenge sein" umgestellt werden koennen, ohne bestehenden Code zu brechen. Ein systematischer Read-Only-Audit (Explore-Agent) aller Lese-/Membership-Zugriffe auf `cell_widgets`/`sequence_field_widgets` (21 Stellen ueber 4 Dateien) hat diese in drei Kategorien eingeteilt: *must-exist* (braucht spaeter echtes Mount-vor-Zugriff, z.B. `ensure_row_visible()`s Abhaengigkeit von live Widget-Geometrie -- bewusst NICHT Teil dieser Aenderung, da die dafuer noetige Layout-Infrastruktur noch nicht existiert), *may-be-absent* (bereits sicher, kein Handlungsbedarf), und *architecture-smell* (Widget-Praesenz wird als Proxy fuer eine eigentliche Domain-Frage missbraucht -- "ist dieses Feld fuer diesen Tag ueberhaupt relevant", "ist das Grid ueberhaupt aufgebaut" -- obwohl die richtige Antwort direkt verfuegbar waere). Nur die zweite Kategorie wurde jetzt behoben: sie ist unabhaengig von noch fehlender Virtualisierungs-Infrastruktur korrekt fixbar und reduziert das Risiko, dass diese Stellen beim spaeteren Bau der Virtualisierung sofort beim ersten Scroll falsch reagieren (unnoetige Voll-Rebuilds, faelschlich geloeschte Selektion).

**Fixes** (7 von 8 gefundenen architecture-smell-Stellen; die achte, `_grid_structure_matches_state()`, bleibt bewusst unveraendert -- ihre korrekte dreiwertige Fassung braucht die Mount-Fenster-Infrastruktur aus der eigentlichen Virtualisierungsarbeit, ein Fix jetzt waere entweder wirkungslos oder verfrueht):

- `ui_intent_controller.py::_copy_selected_cell_to_clipboard()`: liest jetzt `self.app._field_value(day, field_key)` (Domain-Wert) statt `cell_widget.get("1.0", "end-1c")` (Widget-Text). Kopieren ist nur auf `SELECTION_LEVEL_CELL` erreichbar (nie waehrend eines laufenden Edits, s. `intent_clipboard_shortcut`s Level-Gating), beide Werte sind an dieser Stelle also ohnehin identisch -- verifiziert durch Nachlesen von `update_cell()` (`grid_renderer.py`), das exakt denselben `_field_value()`-Aufruf als alleinige Quelle fuer den Widget-Inhalt verwendet. Der sauberste Fix im ganzen Audit: eliminiert die must-exist-Abhaengigkeit komplett, statt sie nur zu verschieben.
- `ui_intent_controller.py::_ensure_selected_cell_visible_after_layout_change()`: prueft jetzt `grid_renderer._field_is_visible_for_day(field_key, day)` statt `(field_key, day_index) not in cell_widgets`. Die alte Pruefung war selbstwidersprechend -- ihr einziger Zweck ist, eine Zelle nach einem Layout-Wechsel per `ensure_*_visible()` in den Sichtbereich zu scrollen, aber der Guard brach genau dann ab, wenn die Zelle (weil gerade ausserhalb des Sichtbereichs) noch kein Widget hatte.
- `grid_renderer.py::_rebuild_grid()`: die Selektions-Loeschung nach einem Rebuild (Modus-/Filterwechsel kann die zuvor selektierte Zelle ungueltig machen) in eine eigene, isoliert testbare Methode `_clear_selection_if_selected_cell_no_longer_visible()` extrahiert, die ebenfalls `_field_is_visible_for_day()` statt `cell_widgets`-Membership prueft (nach dem Vorbild von `_grow_row_minsize_for_cell()`, das denselben Isolations-Testmuster bereits fuer Item 2 etabliert hat).
- `grid_renderer.py::refresh_grid_content()`, `selection_controller.py::_restyle_headers_for_selection_change()`/`clear_selected_cell()`/`set_selected_cell()`: alle vier hatten einen "ist das Grid ueberhaupt aufgebaut"-Bootstrap-Guard, der `cell_widgets`-Wahrheitswert statt `header_labels`-Wahrheitswert prüfte -- obwohl `header_labels` (ein Eintrag pro Tag, unabhaengig von Feld-Sichtbarkeit/-Filtern) das tatsaechlich stabile Signal dafuer ist. Mit restriktiven Zeilenfiltern kann `cell_widgets` legitim leer sein, obwohl das Grid korrekt aufgebaut ist (alter Bug, durch den Fix nebenbei behoben: `refresh_grid_content()` haette in diesem Fall bei jedem Aufruf unnoetig neu gebaut). Die inneren Einzel-Widget-Lookups (bereits defensiv None-geprueft) bleiben unveraendert -- das ist die separate, hier bewusst nicht angefasste must-exist-Frage "existiert das Widget fuer GENAU diese Zelle gerade".

**Tests**: `tests/test_grid_renderer_selection_visibility.py` (5 Faelle, neu, isoliert per `object.__new__(GridRenderer)` ohne echten Tk-Root). `tests/test_selection_controller_column_fast_path.py`: bestehender Test `test_column_selection_falls_back_to_full_refresh_when_grid_not_built` umgestellt auf `header_labels={}` als "nicht aufgebaut"-Simulation (vorher `cell_widgets={}`, was nach dem Fix nicht mehr "nicht aufgebaut" bedeutet); neuer Test `test_column_selection_takes_fast_path_even_when_cell_widgets_empty` beweist explizit die Verhaltensaenderung: bei leerem `cell_widgets`, aber vorhandenen Headern, bleibt der Fast Path aktiv statt auf den vollen Sweep zurueckzufallen. `tests/test_main_window_intents.py`: vier Tests (`test_shortcut_copy_in_cell_selection_copies_selected_cell_content`, `test_shortcut_cut_in_cell_selection_copies_and_clears_selected_cell`, `test_ctrl_down_expands_selected_row_in_cell_mode`, `test_ctrl_up_collapses_selected_row_in_cell_mode`) von `cell_widgets`-Stubs auf `day_columns`/`_field_value`- bzw. `grid_renderer`-Stubs umgestellt. Volle Suite: 707/707 gruen (701 vorher + 6 neue).

### Changed (2026-08-29) — Performance: `DayColumn.stundentyp`/`is_valid_unterricht_file` gecacht (Kursplaner Item 3)

**Anlass**: Fortsetzung der Performance-Analyse. `is_valid_unterricht_file()` (Datei-Existenzcheck) und `stundentyp()` (liest `Path.exists()` transitiv ueber `is_valid_unterricht_file()`, dann `infer_stundentyp(self.yaml)`) wurden pro Grid-Zelle mehrfach unabhaengig neu berechnet: `stundentyp()` selbst ruft `is_valid_unterricht_file()` auf, UND `is_lzk()`/`is_hospitation()`/`is_unterricht()` rufen alle wiederum `stundentyp()` auf — bis zu vier Neuberechnungen derselben beiden Werte fuer dieselbe Zelle.

**Vor der Umsetzung verifiziert statt nur angenommen** (Nutzervorgabe): `DayColumn` ist laut eigenem Klassendocstring ein unveraenderlicher Daten-Snapshot, der bei jeder Aenderung neu gebaut wird — aber das allein reicht nicht, wenn irgendein Code `self.yaml`/`self.link` nach der Konstruktion in-place mutieren wuerde (frozen dataclasses verhindern nur das Neuzuweisen des Feldes selbst, nicht das Mutieren eines darin referenzierten mutablen Dicts). Repoweiter Grep nach `.yaml[...] = `/`.yaml.update(` und allen Stellen, die `day.yaml`/`lesson_yaml`/`yaml_data` als Alias binden: ausschliesslich Lesezugriffe gefunden, keine einzige In-Place-Mutation eines bestehenden `DayColumn`. Die Invariante haelt also tatsaechlich im Code, nicht nur in der Dokumentation.

**Fix**: `is_valid_unterricht_file`/`stundentyp` von parameterlosen Methoden auf `functools.cached_property` umgestellt (funktioniert nachweislich mit `@dataclass(frozen=True)` ohne `slots=True`, da `cached_property` direkt in `instance.__dict__` schreibt statt ueber `__setattr__` — kurz mit einem isolierten Testskript verifiziert, nicht nur als bekanntes Python-Verhalten angenommen). Da beides jetzt Attribute statt Methoden sind, mussten alle zehn echten Aufrufstellen im Code (`day.stundentyp()` -> `day.stundentyp`, `day.is_valid_unterricht_file()` -> `day.is_valid_unterricht_file`) angepasst werden — verteilt ueber `topic_sequence_runs.py`, `daily_course_log_usecase.py`, `export_expected_horizon_usecase.py`, `grid_cell_policy_usecase.py`, `lesson_context_controller.py` sowie zwei interne Aufrufe in `day_column.py` selbst. Ein vergessener Aufrufer waere sofort und laut mit einem `TypeError: '...' object is not callable` aufgefallen, nicht still falsch gelaufen — alle 701 Tests bleiben nach der Anpassung gruen, kein stiller Fehlschlag.

**Tests**: `tests/test_day_column_cached_properties.py` (3 Faelle, neu) — zaehlt Aufrufe der zugrunde liegenden freien Funktionen (`is_valid_unterricht_link`, `infer_stundentyp`) statt nur den zurueckgegebenen Wert zu vergleichen (das wuerde auch bei versehentlich unGecachten Properties gelten): mehrfacher Zugriff auf dieselbe Instanz berechnet nur einmal, `is_lzk()`/`is_hospitation()`/`is_unterricht()` teilen sich einen einzigen `stundentyp`-Cache-Treffer, und der Cache ist Instanzzustand (zwei verschiedene `DayColumn`-Objekte cachen unabhaengig, kein Cross-Talk ueber ein Modul-Global).
...
**Konzeptionelle Entscheidung (Nutzervorgabe)**: nicht nur einen Cache um den bestehenden Voll-Reload legen (das behandelt das Symptom), sondern die eigentliche Ursache beheben — eine Zellbearbeitung soll den Plan gar nicht erst vollstaendig rekonstruieren.
...
**Bewusst nicht angefasst**: `_project_visible_day_columns()` (Sichtbarkeits-Projektion) und `_sync_topic_sequence_plans()` laufen weiterhin bei jedem Aufruf auf der vollen (jetzt aber guenstig gebauten) Liste — reine In-Memory-Listenoperationen, nicht der Engpass.

**Tests**: `tests/test_load_plan_detail_usecase_incremental.py` (7 Faelle: Korrektheitsgleichheit mit Voll-Rebuild, Objekt-Identitaet fuer unveraenderte Zeilen, leere Aenderungsmenge, kein vorheriger Stand, Fallback bei Zeilenzahl-/Lerngruppen-Aenderung, geteilte Link-Datei ueber zwei Zeilen), `tests/test_lesson_repository_yaml_cache.py` (4 Faelle, darunter der vom Nutzer als Pflicht vorgegebene Test: Datei extern mit einem anderen Programm veraendern, waehrend Kursplaner offen ist — muss beim naechsten Lesevorgang sichtbar werden, unveraenderte Dateien bleiben weiterhin aus dem Cache bedient). Alle 688 Tests gruen.

**Nachtraegliche Korrektur beim Benchmarken**: der neue `tools/benchmarks/day_columns_incremental_benchmark.py` zeigte zunaechst nur x1.1–x1.3 zusaetzlichen Gewinn des Einzelzeilen-Patches gegenueber dem Cache allein — der geteilte-Datei-Sicherheitscheck rief fuer JEDE unveraenderte Zeile `resolve_row_link_path()` (Dateisystemzugriff: `Path.resolve()` + `exists()`/`is_file()`, mit Verzeichnis-Fallback-Scan) erneut auf, wodurch der Patch selbst wieder O(Zeilen) teure Aufrufe machte statt O(1). Behoben: der Vergleich nutzt jetzt `previous_day_columns[row_index].link` (bereits aufgeloest, reiner In-Memory-Zugriff) statt erneut aufzuloesen — korrekt, weil eine nicht in `changed_row_indices` enthaltene Zeile ihre Inhalt-Zelle per Definition nicht angefasst hat, ihr Link sich also seit dem Bau von `previous_day_columns` nicht geaendert haben kann (externe Aenderungen an nicht betroffenen Dateien bleiben weiterhin explizit ausserhalb des Scopes, dieselbe Grenze wie beim mtime-Cache). Nach der Korrektur (300 Zeilen, 15 simulierte Edits): Cache allein x1.5, Einzelzeilen-Patch zusaetzlich x44.1, gesamt x64.1 gegenueber dem Zustand vor Item 1 — bei 100 Zeilen x30.6 gesamt. Die wachsende Speedup-Zahl mit der Zeilenanzahl bestaetigt die erwartete O(Zeilen)-zu-O(1)-Verbesserung; alle 7 Korrektheitstests bleiben nach der Aenderung gruen.
