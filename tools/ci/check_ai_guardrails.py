...
SETTINGS_SHORTCUT_SOFT_CHECKS = (
    {
        "label": "open-settings",
        "intent_paths": (
            "kursplaner/adapters/gui/ui_intents.py",
            "kursplaner/adapters/gui/screen_builder.py",
        ),
        "intent_markers": (
            "OPEN_SETTINGS",
            "app.open_settings",
            "command=lambda: self._emit_intent(UiIntent.OPEN_SETTINGS)",
        ),
        "shortcut_paths": (
            "kursplaner/adapters/gui/screen_builder.py",
            "kursplaner/resources/shortcuts/shortcut_guide.json",
        ),
        "shortcut_markers": (
            "<Control-comma>",
            "<Control-,>",
...
def _check_ui_contract_bridge_decommission(errors: list[str]) -> None:
    """Erzwingt Phase-I: ui_contract-Bridges bleiben duenn und re-export-only."""
