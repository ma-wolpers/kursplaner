...
- **Pädagogik-Achievements nach Jahrgangsstufe**: Der Achievements-Tab zeigt jetzt zusätzlich, ob mindestens ein pädagogischer UB in 5./6., 7.-10. und 11.-13. absolviert wurde. Für Mathematik, Informatik und Darstellendes Spiel ist diese Auswertung vorbereitet, aber noch nicht mit konkreten Vorgaben belegt. Die Jahrgangsstufe wird automatisch aus dem Kurs übernommen, zu dem die markierte Einheit gehört — der UB-Markierungsdialog fragt dafür nichts mehr ab, es gibt keine doppelte Pflege mehr.
...
- **Schulweite Ausfälle**: Neues Popup „Schulweite Ausfälle…" (Button in der Kursübersicht, neben „Neuer Kurs"), um einen Ausfall (z. B. Wandertag, pädagogischer Tag) für mehrere Kurse gleichzeitig einzutragen — Grund, Zeitraum und betroffene Jahrgangsstufen (Grundschule/Sek I/Sek II, aufklappbar bis auf einzelne Stufen). Eine Live-Vorschau zeigt bei jeder Eingabe sofort, welche konkreten Kurse/Einheiten betroffen wären; bereits durch einen anderen Ausfall beanspruchte Tage werden markiert. Beim Speichern werden die betroffenen Einheiten automatisch als Ausfall markiert und ihr Inhalt in die nächste freie Lücke verschoben — findet sich keine, wird eine neue Einheit ohne Datum angehängt (im Grid rot umrandet dargestellt, Header + Zelle). Bestehende Einträge lassen sich im selben Fenster bearbeiten oder löschen (nimmt alle Verschiebungen automatisch zurück). Tritt während des Speicherns/Löschens ein Fehler oder Konflikt auf (z. B. eine Zeile wurde inzwischen manuell verändert), fragt ein Dialog nach: Nochmal versuchen, Überspringen, gesamten Vorgang zurückrollen oder (bei bloßen Warnungen) trotzdem durchführen. Ein informativer Hinweis beim Öffnen des Popups zeigt an, falls gespeicherte Einträge nicht mehr zum tatsächlichen Kursplan-Zustand passen.
  ...
  - **Neu/erweitert übernehmen aktive Ausfälle automatisch**: Legt man einen neuen Kurs an oder verlängert einen bestehenden Plan, werden Tage, die ein aktiver schulweiter Ausfall für die passende Jahrgangsstufe beansprucht, direkt als Ausfall markiert — genau wie bereits bei Ferien/Feiertagen.
  - **Datumsfelder erleichtert**: Das Jahr ist jetzt optional (z. B. „14.09." → aktuelles Jahr); das „Bis"-Feld ist bei neuen Einträgen standardmäßig leer und gilt dann als eintägiger Ausfall, wird nur für mehrtägige Ereignisse ausgefüllt. Liegt „Bis" vor „Von", erscheint jetzt eine klare Fehlermeldung statt eines stillen Fehlverhaltens.
  - **Speicherort konfigurierbar**: Die Ausfall-Daten liegen jetzt standardmäßig außerhalb des Programmordners (`~/.kursplaner/`) und lassen sich in den Einstellungen auf einen beliebigen Ort umstellen (z. B. einen synchronisierten Ordner); bestehende Einträge werden beim Wechsel automatisch mitgenommen.
...
### Fixed
- **Automatische Markierung der nächsten Einheit funktionierte nicht mehr**: Beim Öffnen eines Kursplans wurde die als nächstes anstehende Einheit nicht mehr automatisch selektiert/hervorgehoben, und Pfeiltasten-Navigation ohne aktive Auswahl startete nicht mehr an der richtigen Stelle.
- **Themenfolge-Link in "Thema/Ausfall" wurde beim Anlegen/Umbenennen/Einfügen einer Einheit überschrieben**: Betraf jede Aktion, die eine Einheit mit der Planzeile verknüpft (Einheit planen, Einheit umbenennen, Stunde einfügen). Der Link zur Einheit steht jetzt außerdem als Dataview-Verweis in der "Inhalt"-Spalte, sodass dort automatisch der aktuelle Stundenthema-Titel angezeigt wird (statt eines rohen Dateinamens) — ändert sich das Stundenthema, aktualisiert sich die Anzeige in der Planungstabelle automatisch mit. Manuell eingegebener Text in einer bereits verlinkten "Inhalt"-Zelle wird dadurch nicht mehr als eigener Anzeigename übernommen.
- Popups (u. a. „Schulweite Ausfälle…", „Stundenplanänderung…") schlossen sich beim Klick auf „Schließen" mit einem sichtbaren Ruckel-Effekt, als würden die Widgets erst „abgesaugt", statt dass sich das Fenster einfach schließt. Das Popup blendet sich jetzt sofort aus, bevor es abgebaut wird.
...
### Fixed
- **UB-Plan-Tab ("kommende"/"absolvierte" Liste) ignorierte die konfigurierte UB-Vergangenheitsregel**: Nutzte immer den festen Standard-Cutoff (15:00) statt der in den Einstellungen hinterlegten Uhrzeit — unabhängig davon, welchen Wert man dort konfiguriert hatte. Betrifft nur diesen einen Tab; die Kursübersicht und die UB-Achievements nutzten die Einstellung bereits korrekt.
